    ...
    <div className="min-h-screen bg-stone-50 text-neutral-900 font-sans">
        ...
        <div className="max-w-6xl mx-auto px-8 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex gap-8 text-sm text-neutral-500 font-medium">
            {["Courses", "Paths", "Projects", "Community"].map((l) => (
              <a key={l} href="#" className="hover:text-black transition">{l}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/signin")} className="text-sm text-neutral-500 hover:text-black transition">
              Sign in
            </button>
            <button
              onClick={() => navigate("/register")}
              className="bg-black text-white px-6 py-2 rounded-md text-sm font-semibold flex items-center gap-2 transition hover:bg-blue-600"
            >
              Get started
            </button>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-8 py-24">
        <div className={`transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          ...
          <h1 className="font-serif text-[clamp(3rem,7vw,5rem)] leading-tight tracking-tight max-w-3xl mb-6">
            ...
            <em className="text-neutral-400 italic">things that matter.</em>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 border border-neutral-200">
          {[["240K+", "Active learners"], ["1,800+", "Expert instructors"], ["50K+", "Hours of content"], ["94%", "Completion rate"]].map(([n, l], i) => (
            <div key={i} className="p-6 border-r border-b border-neutral-200 last:border-r-0">
      ...
      <section className="max-w-6xl mx-auto px-8 py-20">
        ...
        <div className="divide-y divide-neutral-200 border-t border-neutral-200">
          {courses.map((c) => (
            <div key={c.id} className="grid md:grid-cols-6 gap-6 items-center py-6 hover:bg-neutral-100 transition group px-2">
                ...
                {c.progress > 0 && (
                  <div className="mt-3 max-w-[160px]">
                    ...
                    <div className="h-1 bg-neutral-200 rounded">
                      <div className="h-full bg-blue-600 rounded" style={{ width: `${c.progress}%` }} />
                    </div>
                  </div>
                )}
              </div>
      ...
      {/* TESTIMONIALS */}
      <section className="max-w-6xl mx-auto px-8 py-20">
        <p className="uppercase text-xs tracking-widest text-neutral-400 font-semibold mb-4">Testimonials</p>
        <h2 className="font-serif text-3xl mb-10">Trusted by builders</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <div key={i} className="border border-neutral-200 rounded-xl p-6 bg-white hover:shadow-xl transition">
              <div className="flex gap-1 mb-4">{[...Array(5)].map((_, j) => <Star key={j} size={14} fill="black" />)}</div>
              <p className="text-sm text-neutral-600 mb-6">"{t.content}"</p>
              <div className="flex items-center gap-3">
                <Avatar>
                  <AvatarFallback className="bg-black text-white text-xs font-bold">{t.avatar}</AvatarFallback>
                </Avatar>
                <div>
      ...
      <section className="border-t border-neutral-200 py-24">
        <div className="max-w-6xl mx-auto px-8 grid md:grid-cols-2 gap-16 items-center">
          <h2 className="font-serif text-5xl leading-tight">
            Start building <br />
            <em className="text-neutral-400 italic">something real.</em>
          </h2>
          <div>
            <p className="text-neutral-500 mb-8">Join 240,000+ engineers, designers, and data scientists who chose LearnForge to level up their careers.</p>
            <div className="flex gap-4 mb-6">
              <button
                onClick={() => navigate("/register")}
                className="bg-black text-white px-6 py-3 rounded-md font-semibold flex items-center gap-2 hover:bg-blue-600 transition"
              >
                Get started free <ArrowRight size={16} />
              </button>
              <button className="border border-neutral-300 px-6 py-3 rounded-md text-neutral-600 hover:border-black hover:text-black transition">View pricing</button>
            </div>
